...
describe('Enhanced Therapeutic Messaging', () => {
  describe('Shadow Ability Descriptions', () => {
    it('should provide empathetic and specific descriptions for doubt abilities', () => {
      ...
      const selfQuestioningAbility = whisperOfDoubt.abilities.find(
        (a) => a.id === 'self-questioning',
      );
  ...
  describe('Therapeutic Language Quality', () => {
    it('should use person-first, empowering language', () => {
      ...
      const allGrowthMessages = Object.values(shadowManifestations).map(
        (s) => s.victoryReward.growthMessage,
      );
      const allDescriptions = Object.values(shadowManifestations).flatMap((s) =>
        s.abilities.map((a) => a.description),
      );
