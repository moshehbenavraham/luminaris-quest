  ...
  describe('Scene Outcome Resource Application', () => {
    it('should apply LP rewards for successful non-combat scenes', () => {
      ...
      if (outcome.resourceChanges?.lpChange) {
        const store = useGameStore();
        store.modifyLightPoints(outcome.resourceChanges.lpChange);

        // Verify LP was applied
        expect(store.lightPoints).toBe(outcome.resourceChanges.lpChange);
      }
    });
    ...
    it('should apply SP penalties for failed non-combat scenes', () => {
      ...
      if (outcome.resourceChanges?.spChange) {
        const store = useGameStore();
        store.modifyShadowPoints(outcome.resourceChanges.spChange);

        // Verify SP was applied
        expect(store.shadowPoints).toBe(outcome.resourceChanges.spChange);
      }
    });
    ...
    it('should apply LP rewards for successful combat scenes', () => {
      ...
      if (outcome.resourceChanges?.lpChange) {
        ...
        // Verify LP was applied
        expect(store.lightPoints).toBe(4);
      }
    });
  });
  ...
  describe('Resource Persistence', () => {
    it('should persist resources across multiple scene completions', () => {
      ...
      if (outcome1.resourceChanges?.lpChange) {
        store.modifyLightPoints(outcome1.resourceChanges.lpChange);
      }

      const lpAfterScene1 = store.lightPoints;
      ...
      if (outcome2.resourceChanges?.spChange) {
        store.modifyShadowPoints(outcome2.resourceChanges.spChange);
      }

      expect(store.lightPoints).toBe(lpAfterScene1); // LP should remain
      expect(store.shadowPoints).toBeGreaterThan(0); // SP should be added
    });
    ...
    it('should maintain resources when combat is triggered', () => {
      ...
      store.modifyLightPoints(5);
      store.modifyShadowPoints(2);

      expect(store.lightPoints).toBe(5);
      expect(store.shadowPoints).toBe(2);
      ...
      if (outcome.triggeredCombat && outcome.shadowType) {
        store.startCombat(outcome.shadowType);
      }
