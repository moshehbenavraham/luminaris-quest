...
export function useCombatEffects(options: CombatEffectsOptions = {}): CombatEffectsReturn {
  const { soundEnabled = true, animationsEnabled = true, volume = 0.7 } = options;
  ...
  const playDamageSound = useCallback(
    async (type: 'damage' | 'heal' | 'miss'): Promise<void> => {
      if (!soundEnabled) return;
  ...
  const playStatusSound = useCallback(
    async (type: 'positive' | 'negative' | 'neutral'): Promise<void> => {
      if (!soundEnabled) return;
  ...
  const triggerStatusNotification = useCallback(
    (_message: string, _type: 'success' | 'warning' | 'error' | 'info'): void => {
      if (!animationsEnabled) return;
